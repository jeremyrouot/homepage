...
f = open("data.txt","w+")
for n in range(2,5):
    for M,thind,minX,maxX in list(product(range(100,400,100),range(0,2),range(0,4),range(4,8))):
        entry += 1
        ths = np.random.rand(n+1)
        nomfic='data/data-'+str(entry)+'.txt'
        Xmat,Ymat = gene_data(nomfic,n,M,ths)
        ...
        ysol = hdex(xsanscible,thetasol)[0]
